 ...
 * @see https://hawkbit.eclipse.dev/#/direct-device-integration-api
 */
...
{
        g_return_if_fail(data <= CURL_LOCK_DATA_LAST);
...
static void set_default_curl_opts(CURL *curl)
{
        ...
        if (g_once_init_enter(&curl_share_initialized)) {
                /* initialize mutexes for different types of data that curl uses locks for */
                for (gint i = 0; i <= CURL_LOCK_DATA_LAST; i++)
                        g_mutex_init(&curl_share_locks[i]);

                curl_share = curl_share_init();
                g_assert_nonnull(curl_share);
 ...
 *
 * @see https://hawkbit.eclipse.dev/rest-api/ddi.html#tag/DDI-Root-Controller/operation/postDeploymentBaseActionFeedback
 *
 ...
 * level.
 *
 * @see https://hawkbit.eclipse.dev/rest-api/ddi.html#tag/DDI-Root-Controller/operation/putConfigData
 *
 * @param[out] error Error
